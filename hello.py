...
#allUrl='https://airecsim-cdn.huohua.cn/rec-v2/REC2007405706/latest.json'
allUrl='https://airecqa-cdn.huohua.cn/rec-v2/REC2009005505/latest.json'
#recCode=input('请输入reccode: ')
#environment=input('请输入录像环境： ')
#allUrl=' https://airec'+environment+'-cdn.huohua.cn/rec-v2/'+recCode+'/latest.json'
...
for all in json3:
    ...
    mydict["页数"] = '第'+str(n)+'页'
    n=n+1
    mydict["label"]= all['label']
    mydict["断点标记"]= all['metadata']['guid']
    ...
    for edge in all['edges']:
        if  'expectations' in  edge['metadata'].keys():
            if edge['label'] == '识别结果1'and edge['metadata']['expectations']!='':
                mydict["识别结果1"]= edge['metadata']['expectations']
            elif edge['label'] == '识别结果2'and edge['metadata']['expectations']!='':
                mydict["识别结果2"]= edge['metadata']['expectations']
            elif edge['label'] == '识别结果3' and edge['metadata']['expectations']!='' :
                mydict["识别结果3"]= edge['metadata']['expectations'] 
        else:
           print('')
    b.append(mydict)
#print(a)
